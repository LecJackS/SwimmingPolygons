"""Render scripted V8 color-communication scenarios."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render scripted V8 color-communication scenarios.")
    ...
    *,
    ...
    if scenario == "sensor_overlay_demo":
        ...
        return lambda step_idx: {
            agent_id: {
                "motion": np.array([0.4, 0.2 if agent_id == env.focus_agent_id else 0.0], dtype=np.float32),
    ...
    if scenario == "color_capture_demo":
        ...
        return lambda step_idx: {
            agent_id: {
                "motion": np.array([0.7, 0.0], dtype=np.float32),
        ...
        return lambda step_idx: {
            agent_id: {
                "motion": np.array(
                    [
                        0.75,
                        0.6 * np.sin((0.08 * step_idx) + (0.55 * idx)),
                    ],
                    dtype=np.float32,
                ),
