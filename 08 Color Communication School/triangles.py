"""V8 environment: colored pellets, colored fish, and local communication."""
...
class CommunicatingSchoolEnv(MultiAgentEnv):
    """Colored multi-agent school with one shared trainable policy."""
    ...
    def __init__(
        ...
        if self.num_red_fish <= 0 or self.num_blue_fish <= 0:
            raise ValueError("num_red_fish and num_blue_fish must be > 0.")
        if self.num_red_pellets <= 0 or self.num_blue_pellets <= 0:
            raise ValueError("num_red_pellets and num_blue_pellets must be > 0.")
        ...
        if self.sector_num != 6:
            raise ValueError("V8 keeps a fixed 6-sector observation contract.")
        if self.num_message_tokens != 4:
            raise ValueError("V8 keeps a fixed 4-token communication contract.")
        ...
        obs_low = np.concatenate(
            [
                ...
                np.array([0.0] + [-1.0] * 9 + [0.0, 0.0], dtype=np.float32),
            ]
        )
        obs_high = np.ones(48, dtype=np.float32)
    ...
    def _compute_observation_bundle(self, agent_ids: list[str] | tuple[str, ...] | None = None) -> dict[str, np.ndarray]:
        ...
        prev_actions_all = np.asarray([self.fish_states[agent_id].prev_action for agent_id in self.possible_agents], dtype=np.float32)
        ...
        selected_prev_actions = prev_actions_all[agent_indices]
        selected_prev_message_tokens = prev_message_tokens_all[agent_indices]
        selected_omega = omega_all[agent_indices]
        ...
        for row_idx, agent_id in enumerate(agent_ids):
            ...
            proprioception = np.array(
                [
                    float(teams[row_idx]),
                    np.clip(root_velocity_body[row_idx, 0] / dynamics.max_speed, -1.0, 1.0),
                    np.clip(root_velocity_body[row_idx, 1] / dynamics.max_speed, -1.0, 1.0),
                    np.clip(selected_omega[row_idx] / dynamics.max_angular_speed, -1.0, 1.0),
                    np.clip(selected_joint_angles[row_idx, 0] / actuation.joint_limit, -1.0, 1.0),
                    np.clip(selected_joint_angles[row_idx, 1] / actuation.joint_limit, -1.0, 1.0),
                    np.clip(selected_joint_velocities[row_idx, 0] / actuation.joint_max_speed, -1.0, 1.0),
                    np.clip(selected_joint_velocities[row_idx, 1] / actuation.joint_max_speed, -1.0, 1.0),
                    np.clip(selected_prev_actions[row_idx, 0], -1.0, 1.0),
                    np.clip(selected_prev_actions[row_idx, 1], -1.0, 1.0),
    ...
    def _compute_substep_dynamics(
        ...
        actuation = self._compute_actuation(state, action, dt, swim_phase, include_debug=False)
        ...
        total_force = np.asarray(wrench["total_force"], dtype=np.float32) + gait_propulsion_force
        ...
        return {
            ...
            "root_acceleration": (total_force / float(dynamics.mass)).astype(np.float32),
    ...
    def get_dynamics_breakdown(
        ...
        *,
        ...
        actuation = self._compute_actuation(state, action, dt, swim_phase, include_debug=True)
        predicted_state = ArticulatedFishState(
    ...
    def _integrate_substep(
    ...
    ) -> tuple[ArticulatedFishState, float]:
        dynamics = self.fish_preset.dynamics
        dynamics_step = self._compute_substep_dynamics(state, action, dt, swim_phase)
        ...
        return next_state, float(dynamics_step["swim_phase_next"])
    ...
    def _compute_next_state(
        ...
        for _ in range(substeps):
            next_state, next_swim_phase = self._integrate_substep(
                agent_id,
                next_state,
    ...
    def get_reward_breakdown(
        ...
        *,
        ...
        pellet_reward_total = float(self.pellet_reward * food_eaten_this_step)
    ...
    def _food_capture_assignments(self) -> tuple[dict[str, int], np.ndarray]:
        fish_positions = self._all_root_positions()
        ...
        return food_eaten_by_agent, captured_mask
    ...
    def step(self, action_dict: dict[str, Any]):
        ...
        for agent_id, action in actions.items():
            next_state, next_swim_phase = self._compute_next_state(
                agent_id,
                action["motion"],
                int(action["message"]),
            )
        ...
        food_eaten_by_agent, eaten_mask = self._food_capture_assignments()
    ...
    def get_debug_snapshot(self, agent_id: str | None = None) -> Dict[str, Any]:
        ...
        dynamics_breakdown = self.get_dynamics_breakdown(
            ...
            state.prev_action,
            state=state,
            swim_phase=self.swim_phases[agent_id],
    ...
    def _update_render_artists(self) -> None:
        ...
        if self.status_text is not None:
            ...
            self.status_text.set_text(
                f"V8 | step={self.timestep}/{self.time_limit} | red={self.team_food_eaten_episode[RED_TEAM]} "
