...
class OctopusEnv(gym.Env):
    ...
    def __init__(
        ...
        self.base_step_penalty = -0.15
        self.progress_reward_scale = 2.5
        self.progress_reward_clip = 0.3
        ...
        self.last_reward_breakdown: Dict[str, float | bool] = {
            "base_step_penalty": float(self.base_step_penalty),
            "progress_delta": 0.0,
            "progress_reward": 0.0,
            "success_bonus": 0.0,
            "total_reward": 0.0,
    ...
    def set_debug_state(
        ...
        """Force a deterministic simulator state for env-only diagnostics."""
        self.fish_state = FishState(
            position=np.asarray(position, dtype=np.float32).copy(),
            velocity=np.asarray(velocity, dtype=np.float32).copy(),
            theta=float(theta),
            omega=float(omega),
            prev_turn_action=int(prev_turn_action),
            prev_push_action=int(prev_push_action),
        )
        self.food_position = np.asarray(food_position, dtype=np.float32).copy()
    ...
    def get_reward_breakdown(self, prev_dist_to_food: float, dist_to_food: float) -> Dict[str, float | bool]:
        ...
        progress_reward = float(
            np.clip(
                self.progress_reward_scale * progress_delta,
                -self.progress_reward_clip,
                self.progress_reward_clip,
            )
        )
        terminated = bool(dist_to_food < self.success_distance)
        truncated = bool((not terminated) and (self.timestep >= self.time_limit))
        success_bonus = float(self.success_bonus if terminated else 0.0)
        total_reward = float(self.base_step_penalty + progress_reward + success_bonus)
        return {
            "base_step_penalty": float(self.base_step_penalty),
    ...
    def get_debug_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            "position": self.fish_state.position.astype(np.float32).copy(),
            "velocity": self.fish_state.velocity.astype(np.float32).copy(),
            "theta": float(self.fish_state.theta),
            "omega": float(self.fish_state.omega),
            "food_position": self.food_position.astype(np.float32).copy(),
            "target_relative": rel.astype(np.float32).copy(),
            "distance_to_food": float(np.linalg.norm(rel)),
            "timestep": int(self.timestep),
            "prev_turn_action": int(self.fish_state.prev_turn_action),
    ...
    def _compute_reward_flags(self, prev_dist_to_food: float, dist_to_food: float) -> Tuple[float, bool, bool]:
        reward_breakdown = self.get_reward_breakdown(prev_dist_to_food, dist_to_food)
        ...
        self.last_progress_reward = float(reward_breakdown["progress_reward"])
    ...
    def _build_info(self, dist_to_food: float, terminated: bool) -> Dict[str, float | bool | str]:
        return {
            ...
            "last_progress_reward": float(self.last_progress_reward),
    ...
    def step(self, action):
        ...
        prev_dist_to_food = float(np.linalg.norm(self._target_relative_vector()))
        self.fish_state = self._compute_next_state(turn_idx, push_idx)
        self.timestep += 1

        dist_to_food = float(np.linalg.norm(self._target_relative_vector()))
        reward, terminated, truncated = self._compute_reward_flags(prev_dist_to_food, dist_to_food)
        obs = self._get_obs()
