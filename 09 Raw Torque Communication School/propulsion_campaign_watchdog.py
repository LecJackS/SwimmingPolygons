"""Watch and relaunch the V9 propulsion campaign controller."""
...
CONTROLLER_STDOUT = SCRIPT_DIR / "propulsion_campaign_controller.out.log"
CONTROLLER_STDERR = SCRIPT_DIR / "propulsion_campaign_controller.err.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Watch and relaunch the V9 propulsion campaign controller.")
...
def find_controller_processes(manifest_path: Path) -> list[dict[str, Any]]:
    command = (
        ...
        + " $_.Name -match 'python' -and $_.CommandLine -match 'train_until_propulsion_target.py' -and $_.CommandLine -match $rx "
...
def launch_controller(args: argparse.Namespace, *, resume_existing: bool) -> subprocess.Popen[str]:
    command = [
        ...
        str((SCRIPT_DIR / "train_until_propulsion_target.py").resolve()),
        "--manifest-path",
