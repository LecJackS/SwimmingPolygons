"""Delayed finish-check watcher for V6 continuous foraging runs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Watch a V6 foraging training run and auto-evaluate when it finishes.")
    parser.add_argument("--pid-file", type=str, default="./baseline_v6_foraging.pid")
    parser.add_argument("--checkpoint-root", type=str, default="./rllib_checkpoints_baseline_v6_foraging")
    parser.add_argument("--log-path", type=str, default="./baseline_v6_foraging.out.log")
    parser.add_argument(
        "--check-offset-minutes",
        type=str,
        default="90,120",
        help="Comma-separated offsets in minutes from watcher start.",
    )
    parser.add_argument("--primary-eval-episodes", type=int, default=10)
    parser.add_argument("--secondary-eval-episodes", type=int, default=5)
...
def run_auto_evaluation(
    *,
    ...
    primary_base = auto_eval_dir / f"{primary_checkpoint.name}_summary"
    run_batch_evaluation(
        ...
        seed=args.seed,
        summary_json_path=primary_base.with_suffix(".json"),
        summary_csv_path=primary_base.with_suffix(".csv"),
        watcher_log_path=watcher_log_path,
    )

    secondary_base = None
    if secondary_checkpoint is not None:
        secondary_base = auto_eval_dir / f"{secondary_checkpoint.name}_summary"
        run_batch_evaluation(
            python_executable=args.python_executable,
            ...
            summary_json_path=secondary_base.with_suffix(".json"),
            summary_csv_path=secondary_base.with_suffix(".csv"),
            watcher_log_path=watcher_log_path,
        )
    ...
    manifest = {
        ...
        "primary_summary_json": str(primary_base.with_suffix(".json").resolve()),
        "primary_summary_csv": str(primary_base.with_suffix(".csv").resolve()),
        "secondary_summary_json": str(secondary_base.with_suffix(".json").resolve()) if secondary_base else None,
        "secondary_summary_csv": str(secondary_base.with_suffix(".csv").resolve()) if secondary_base else None,
...
def main() -> None:
    ...
    write_watcher_log(
        watcher_log_path,
        f"watcher_started pid_file={pid_file} checkpoint_root={checkpoint_root} log_path={log_path}",
    )
